...
def FastAttention(inputs,filters,batch_size,input_shape):
    ...
    q=layers.Conv2D(64,1)(inputs) # Aquí vale 32 según el paper
    q_norm = layers.LayerNormalization()(q)

    k=layers.Conv2D(64,1)(inputs)
    k_norm = layers.LayerNormalization()(k) # Aquí vale 32 según el paper

    v=layers.Conv2D(filters,1,activation='relu')(inputs)

    #dot_kv=layers.multiply([k_norm,v])
    #dot_qkv=layers.multiply([q_norm,dot_kv])
    dot_qkv=fast_attention_layer(batch_size,(input_shape[1],input_shape[2],64),(input_shape[1],input_shape[2],filters))([q_norm,k_norm,v])
...
def generate_model(input_shape,backbone,layer,batch_size):
    ...
    relu=layers.Softmax()(fuseup1)

    model=keras.models.Model(inputs=[backbone.input],outputs=[relu])
