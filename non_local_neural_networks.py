...
metrics=['accuracy']#,tf.keras.metrics.MeanIoU(OUTPUT_CHANNELS)]
callbacks = [
...
def FastAttention(inputs,filters,batch_size,input_shape):
    ...
    q=tf.keras.layers.Conv2D(64,1)(inputs) # Aquí vale 32 según el paper
    q_norm = tf.keras.layers.LayerNormalization()(q)

    k=tf.keras.layers.Conv2D(64,1)(inputs)
...
history = model.fit(x=train_gen,batch_size=batch_size,steps_per_epoch=27358/batch_size,validation_steps=1160/batch_size, epochs=epochs, callbacks=callbacks,
