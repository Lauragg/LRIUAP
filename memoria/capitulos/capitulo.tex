...
Sea $V$ un campo aleatorio (random field) y los datos de entrada $v$ una realización de $V$. Sea $Z$ una secuencia de variables aleatorias con $Z_i={X_i,Y_i}$ con $Y_i=V(i)$ es real valuada y $X_i=V(I\{i}) \R^p$ valuada donde $I$ representa el conjunto de posiciones. Una operación no local genérica será un estimador de la esperanza condicionada $E[Y_i|X_i=v(I\{i})]$.

\begin{teorema}
Sea $Z={V(N_i\{i},V(i)}_{i\gr 1}$ un proceso estocástico estrictamente estacionario y mezclado \emph{strictly stationary and mixing process}. Sea $f^n$ una operación no local genérica aplicada a la secuencia $Z_n={V(N_i\{i},V(i)}_{i=1}^n$. Entonces, $$ |f^n(j)-[Y_j|X_j=v(I\{j})]| \to 0 \; a.s \;  \forall j \in{1,...,n}$$.
\end{teorema}
