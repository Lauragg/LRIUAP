...
callbacks = [
    ...
    ModelCheckpoint('model-con-preprocess1.h5', verbose=1, save_best_only=True, save_weights_only=False)
]
...
# Upsampling and establishing the skip connections
    for up, skip,channel in zip(up_stack, skips,channels):
        ...
        print("FA: "+str(fast_attention.shape)+"\nx: "+str(x.shape))
        x = concat([x, fast_attention])

    x=up_stack[-1](x)
    #fast_attention=FastAttention(s,64,batch_size,s.shape)
    #s=tf.keras.layers.Conv2DTranspose(64,3,strides=2,padding='same')(fast_attention)
    #concat=tf.keras.layers.Concatenate()
    #print(s.shape)
    #print(x.shape)
    #x=concat([x,s])
...
model.save('./modelos/attention_buena1.h5')
